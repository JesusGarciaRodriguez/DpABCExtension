...
public class RangeVerifier {
     ...
     * @return
     */
    public RangePredicateVerificationResult verifyRangePredicate(PedersenBase base, RangePredicateToken token, AttributeDefinition definition, Predicate predicate, String context){
        ...
        switch (predicate.getOperation()){
            case LESSTHANOREQUAL:
                return verifyRangePredicateLessThan(base,token,definition,predicate.getValue(), context);
            case GREATERTHANOREQUAL:
                return verifyRangePredicateGreaterThan(base,token,definition,predicate.getValue(), context);
            case INRANGE:
                ...
                return verifyRangePredicateInRange(base,token,definition,predicate.getValue(),predicate.getExtraValue(), context); //We could perform check/choose the lower and upper bound from predicate instead of relying on definition
            default:
                throw new IllegalArgumentException("Predicate operation not for range proof: "+predicate.getOperation());
        }
    }

    private RangePredicateVerificationResult verifyRangePredicateLessThan(PedersenBase base, RangePredicateToken token, AttributeDefinition definition, Attribute upperBound, String context) {
        ...
        boolean resultUpper=verifier.verify(rangeProofBase,base,vPrime,token.getProofUpperBound(), context); //Check x<=upperBound
        return (resultUpper? RangePredicateVerificationResult.VALID : RangePredicateVerificationResult.INVALID);
    }

    private RangePredicateVerificationResult verifyRangePredicateGreaterThan(PedersenBase base, RangePredicateToken token, AttributeDefinition definition, Attribute lowerBound, String context) {
        ...
        boolean resultLower=verifier.verify(rangeProofBase,base,vPrime,token.getProofLowerBound(), context); //Check x>=lowerBound
    ...
    private RangePredicateVerificationResult verifyRangePredicateInRange(PedersenBase base, RangePredicateToken token, AttributeDefinition definition, Attribute lowerBound, Attribute upperBound, String context) {
        int m= Util.nextPowerOfPowerOfTwo(definition.toBigIntegerRepresentation(upperBound));
        RangeProofBase rangeProofBase= Utils.generateRangeProofBase(m,salt,builder);
        RangeProofVerifier verifier=new RangeProofVerifier(builder);
        ZpElement offset= two.pow(m).sub(one).sub(builder.getZpElementFromAttribute(upperBound,definition));
        ...
        boolean resultLower=verifier.verify(rangeProofBase,base,vPrimePrime,token.getProofLowerBound(), context); //Check x>=lowerBound
        boolean resultUpper=verifier.verify(rangeProofBase,base,vPrime,token.getProofUpperBound(), context); //Check x<=upperBound
