...
//		PresentationToken zkPT=credentialClientModule.generatePresentationToken(policy);
	...
	public void testInvalidVerifications() throws Exception {
        ...
        PresentationToken zkPTbadRangeTokens=new PresentationToken(zkPTrange.getEpoch(),zkPTrange.getRevealedAttributes(),zkPTrange.getZkToken(),null,null,null,null);
        ...
        PresentationToken zkPTbadSignatureEpochModified=new PresentationToken(zkPTrange.getEpoch()+2,zkPTrange.getRevealedAttributes(),zkPTrange.getZkToken(),zkPTrange.getRangeTokens(),null,null,null);
        // RangePredicate proof failed
        Map<String, RangePredicateToken> badRangeTokens=zkPTrange.getRangeTokens().entrySet().stream()
                .collect(Collectors.toMap(e->e.getKey(),e->new RangePredicateToken(e.getValue().getProofUpperBound(),e.getValue().getProofLowerBound(),e.getValue().getCommitV())));
        PresentationToken zkPTbadRangeToken=new PresentationToken(zkPTrange.getEpoch(),zkPTrange.getRevealedAttributes(),zkPTrange.getZkToken(),badRangeTokens,null,null,null);
